...
const Nav = () => {
  const [open, setOpen] = useState(false); // State for controlling the menu

  return (
    <>
      <header className="border-b border-gray-300 py-2 bg-darkBlue relative">
        <div className="flex justify-between items-center xl:max-w-7xl xl:mx-auto max-w-full px-[8%] flex-wrap">
          ...
          {open && (
            <div className="fixed inset-0 z-20 bg-black opacity-50" onClick={() => setOpen(false)} />
          )}

          {/* Nav Links with transitions */}
          <nav
            className={`${
              open ? "block" : "hidden"
            } lg:flex lg:items-center lg:w-auto w-full transition-transform transform duration-700 ease-in-out ${
              open ? "translate-y-0 opacity-100" : "-translate-y-10 opacity-0"
            } absolute top-full left-0 bg-darkBlue lg:static lg:bg-transparent z-30`}
          >
